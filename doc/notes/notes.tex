...
\begin{equation}\label{cc_update:chi2}
    \chi^2[\mathbf{h}] = \frac{1}{N} \sum_{n=1}^{N}
        \frac{|\hat K A[\mathbf{h}] - g_n|^2}{\delta^2_n},
\end{equation}
where $g_n$ is the discretized right-hand side of the analytic continuation problem, and $\delta_n$ are respective estimated error bars on the input data. Action of the integral kernel $\hat K$ on a configuration $A[\mathbf{h}]$ is expressed in terms of the integrated kernel $\Lambda$,
\begin{equation}
...
\begin{equation}
    \chi^2[\mathbf{h}] =
    \frac{1}{N} \sum_{n=1}^N \frac{1}{\delta_n^2}
...
with
\begin{align}
    (\hat O_{\chi^2})_{kk'} &= \frac{1}{N} \sum_{n=1}^N \frac{1}{\delta_n^2}
        \mathrm{Re}\left[
        \Lambda^*_n(c_k, w_k) \Lambda_n(c_{k'}, w_{k'})
        \right],\\
    (\mathbf{f}_{\chi^2})_k &= \frac{1}{N} \sum_{n=1}^N \frac{1}{\delta_n^2}
...
\begin{enumerate}
    ...
    \item If $d(I, I-1) < \min_n |\delta_n/g_n|$, then convergence is reached and iterations are terminated. $g_n$ is the discretized right-hand side of the analytic continuation problem, and $\delta_n$ are respective estimated error bars on the input data.
