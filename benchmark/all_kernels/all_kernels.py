...
def run_som_and_save_error_bars(kind,
                                mesh,
                                g,
                                error_bars,
                                norms,
    ...
    print_master("Elapsed time: %f s" % elapsed_time)
    if mpi.is_master_node():
        if mesh not in arch[kind]:
            arch[kind].create_group(mesh)
        gr = arch[kind][mesh]
        gr["input"] = g
        if "error_bars" not in gr:
...
def run_som_and_save_cov_matrix(kind,
                                mesh,
                                g,
                                cov_matrix,
                                norms,
...
def g_iw_model(iw):
    def kern(e):
        return 1 / (iw - e)
    return np.diag(g_norms) * quad_complex(lambda e: dos(e, 1) * kern(e),
                                           -1, 3,
                                           points=[0])
...
def g_tau_model(tau):
    def kern(e):
        return -np.exp(-tau * e) / (1 + np.exp(-beta*e))
    return np.diag(g_norms) * quad(lambda e: dos(e, 1) * kern(e),
                                   -1, 3,
                                   points=[0])[0].real
...
def g_l_model(ell):
    def kern(e):
        return -beta * np.sqrt(2 * ell + 1) * ((-np.sign(e)) ** ell) * \
               spherical_in(ell, np.abs(e)*beta/2) / (2 * np.cosh(e*beta/2))
    return np.diag(g_norms) * quad(lambda e: dos(e, 1) * kern(e),
                                   -1, 3,
                                   points=[0])[0].real
...
for mesh, g, error_bars, cov_matrix in (
    ...
    run_som_and_save_error_bars("FermionGf",
                                mesh,
                                g,
                                error_bars,
                                g_norms,
                                (-5, 5))
    run_som_and_save_cov_matrix("FermionGf",
                                mesh,
                                g,
                                cov_matrix,
                                g_norms,
...
def g_iw_model(iw):
    def kern(e):
        return 1 / (iw - e)
    return np.diag(g_norms) * quad_complex(lambda e: dos(e, 0) * kern(e),
                                           -2, 2,
                                           points=[0])
...
def g_tau_model(tau):
    def kern(e):
        return -np.exp(-tau * e) / (1 + np.exp(-beta*e))
    return np.diag(g_norms) * quad(lambda e: dos(e, 0) * kern(e),
                                   -2, 2,
                                   points=[0])[0].real
...
for mesh, g, error_bars, cov_matrix in (
    ...
    run_som_and_save_error_bars("FermionGfSymm",
                                mesh,
                                g,
                                error_bars,
                                g_norms,
                                (-5, 5))
    run_som_and_save_cov_matrix("FermionGfSymm",
                                mesh,
                                g,
                                cov_matrix,
                                g_norms,
...
def chi_iw_model(iw):
    if iw == 0:
        ...
        def kern(e):
            return -e / (iw - e) / np.pi
    return np.diag(chi_norms) * quad_complex(lambda e: dos(e, 1) * kern(e),
                                             -1, 3,
                                             points=[0])
...
for mesh, chi, error_bars, cov_matrix in (
    ...
    run_som_and_save_error_bars("BosonCorr",
                                mesh,
                                chi,
                                error_bars,
                                chi_norms,
                                (-5, 5))
    run_som_and_save_cov_matrix("BosonCorr",
                                mesh,
                                chi,
                                cov_matrix,
                                chi_norms,
...
def chi_auto_tau_model(tau):
    def kern(e):
        return e * (np.exp(-tau * e) + np.exp(-(beta-tau) * e)) / \
            (1 - np.exp(-beta*e)) / np.pi
    return np.diag(chi_auto_norms) * quad(lambda e: 2*dos(e, 0) * kern(e),
                                          0, 2)[0].real
...
def chi_auto_l_model(ell):
    def kern(e): return beta * (1 + (-1) ** ell) * np.sqrt(2 * ell + 1) \
        * e * spherical_in(ell, e*beta/2) / np.sinh(e*beta/2) / (2*np.pi)
    return np.diag(chi_auto_norms) * quad(lambda e: 2*dos(e, 0) * kern(e),
                                          0, 2)[0].real
...
for mesh, chi_auto, error_bars, cov_matrix in (
    ...
    run_som_and_save_error_bars("BosonAutoCorr",
                                mesh,
                                chi_auto,
                                error_bars,
                                chi_auto_norms,
                                (-5, 5))
    run_som_and_save_cov_matrix("BosonAutoCorr",
                                mesh,
                                chi_auto,
                                cov_matrix,
                                chi_auto_norms,
...
def g_zt_iw_model(iw):
    def kern(e):
        return 1 / (iw - e)
    return np.diag(g_zt_norms) * quad_complex(lambda e: 2*dos(e, 0) * kern(e),
                                              0, 2)
...
def g_zt_tau_model(tau):
    def kern(e):
        return -np.exp(-tau * e)
    return np.diag(g_zt_norms) * quad(lambda e: 2*dos(e, 0) * kern(e),
                                      0, 2)[0].real
...
def g_zt_l_model(ell):
    def kern(e):
        return beta * ((-1) ** (ell + 1)) * np.sqrt(2 * ell + 1) * \
            spherical_in(ell, e*beta/2)*np.exp(-e*beta/2)
    return np.diag(g_zt_norms) * quad(lambda e: 2*dos(e, 0) * kern(e),
                                      0, 2)[0].real
...
for mesh, g_zt, error_bars, cov_matrix in (
    ...
    run_som_and_save_error_bars("ZeroTemp",
                                mesh,
                                g_zt,
                                error_bars,
                                g_zt_norms,
                                (0, 5))
    run_som_and_save_cov_matrix("ZeroTemp",
                                mesh,
                                g_zt,
                                cov_matrix,
                                g_zt_norms,
