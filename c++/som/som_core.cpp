...
som_core::som_core(gf_const_view<imtime> g_tau, gf_const_view<imtime> S_tau,
 ...
 kind(kind), mesh(g_tau.mesh()), rhs(input_data_r_t()), error_bars(input_data_r_t()),
...
som_core::som_core(gf_const_view<imfreq> g_iw, gf_const_view<imfreq> S_iw,
                   observable_kind kind, vector<double> const& norms) :
 kind(kind), mesh(g_iw.mesh()), rhs(input_data_c_t()), error_bars(input_data_c_t()),
...
som_core::som_core(gf_const_view<legendre> g_l, gf_const_view<legendre> S_l,
                   observable_kind kind, vector<double> const& norms) :
 kind(kind), mesh(g_l.mesh()), rhs(input_data_r_t()), error_bars(input_data_r_t()),
...
triqs::arrays::array<dcomplex, 3> som_core::compute_tail(int max_order) const {
 ...
 for(int i : range(gf_dim))
  tail(range(), i, i) = som::compute_tail(kind,
                                          results[i],
