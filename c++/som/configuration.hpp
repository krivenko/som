...
public:
 ...
 friend void h5_read(h5::group gr, std::string const& name, configuration& c, cache_index & ci_) {
  using triqs::arrays::array;
  array<double,2> data;
  h5_read(gr, name, data);
  for(int i = 0; i < first_dim(data); ++i) c.insert({data(i,0), data(i,1), data(i,2), ci_});
 }
};
