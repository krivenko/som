...
void update_O_mat(array<double, 2>& A_j_local_block,
  ...
  O_mat += U * ones<double>(O_mat.shape());
...
vector<double> cc_protocol(
    ...
    global_index_map const& index_map,
    mpi::communicator const& comm) {
  int const my_J = good_solution_indices.size();

  int const my_rank = comm.rank();
  auto const my_j_start = index_map.range_start(my_rank);
  auto const my_j_size = index_map.range_size(my_rank);
