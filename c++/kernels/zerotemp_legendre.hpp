...
template<> class kernel<ZeroTemp,legendre> :
           public kernel_base<kernel<ZeroTemp,legendre>, array<double,1>> {
 ...
 struct evaluator {
  ...
  evaluator(int l, double x0_start) : sqrt_pref((l%2 ? 1 : -1) * std::sqrt(2*l+1)) {
  ...
  double operator()(double x) const {
   double val = (x <= x0) ?
                low_energy_spline(x) :
                (low_energy_x0 - high_energy_x0 + (std::log(x) + high_energy_pol(1/x)));
...
public:
 ...
 void apply(rectangle const& rect, result_type & res) const {
  ...
  for(auto l : mesh) res(l.index()) = rect.height * (Lambda(l, e2) - Lambda(l, e1));
 }

 friend std::ostream & operator<<(std::ostream & os, kernel const& kern) {
  os << "A(\\epsilon) -> G_{T=0}(l), ";
  os << "\\tau_{max} = " << kern.tau_max << ", "
     << kern.mesh.size() << " Legendre coefficients.";
  return os;
 }
