...
#include <som/numerics/expabs_distribution.hpp>
...
TEST(expabs_distribution, main) {
  ...
  for(double gamma : {1.0, 2.0}) {
    ...
    for(auto xminmax :
        {xminmax_t(-5.0, -2.0), xminmax_t(-3.0, 5.0), xminmax_t(1.5, 4.0)}) {
      ...
      double x2av_ref =
          N / (gL * gL * gL) *
          (-std::copysign(1, xminmax.first) *
               (2 - poly(gLxmina, {2, 2, 1}) * std::exp(-gLxmina)) +
           std::copysign(1, xminmax.second) *
               (2 - poly(gLxmaxa, {2, 2, 1}) * std::exp(-gLxmaxa)));
      double x3av_ref =
          N / (gL * gL * gL * gL) *
      ...
      double x4av_ref =
          N / (gL * gL * gL * gL * gL) *
          (-std::copysign(1, xminmax.first) *
               (24 - poly(gLxmina, {24, 24, 12, 4, 1}) * std::exp(-gLxmina)) +
           std::copysign(1, xminmax.second) *
               (24 - poly(gLxmaxa, {24, 24, 12, 4, 1}) * std::exp(-gLxmaxa)));
