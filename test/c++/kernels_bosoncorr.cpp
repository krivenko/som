...
TEST(BosonCorr, imtime) {
 ...
 h5::file file("bosoncorr_imtime.h5", H5F_ACC_RDWR);
 ...
 for(double b : beta) {
  ...
  gf_mesh<imtime> mesh(b, Boson, second_dim(results));
  kernel<BosonCorr,imtime> kern(mesh);
  ci.invalidate_all();
...
TEST(BosonCorr, imfreq) {
 ...
 h5::file file("bosoncorr_imfreq.h5", H5F_ACC_RDWR);
